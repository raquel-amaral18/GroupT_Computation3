...
def game(SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    while carryOn:
        ...
        """ 
          CONECTAR GITHUB TO VS CODE
                            
            2º - Submit something to main
            
            5.(-1)º - Criar Random Powerup + Criar Magnet (make car width street size only for the collisiobn with ) 
                    + corrigir mask coin collision size change
                            
            6º - Perceber coins e fzr com q elas n venham juntas
                                                
            10º- coin sparkles
        """

         # Power up picker --> probability:            Only active when we have no stored powerups
        if all(not powerup.active for powerup in incoming_powerups_list) and stored_powerup is None:
            ...
            # Slow Down
            elif 54 <= spawn_prob < 75:
                power_up_slowing.active = True
            # Invincibility
            elif 77 <= spawn_prob <= 88:
        ...
        for powerup in incoming_powerups_list:
            ...
            if powerup.active:
                ...
                if pygame.sprite.collide_mask(playerCar, powerup) is not None:
                    ...
                    if any(p.powered_up for p in incoming_powerups_list if
