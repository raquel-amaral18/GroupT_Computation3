...
def game(SCREEN_WIDTH, SCREEN_HEIGHT, elapsed_time, lives, coin_counter):
    ...
    # VEHICLES:
    # Player's car
    playerCar = PlayerCar("Images/00C.png", 60, 3)
    ...
    # Opponent cars
    car1 = IncomingCars("Images/04C.png", 60, 2, road_x + (lane_width - 50) // 2)
    ...
    while carryOn:
        ...
        for event in pygame.event.get():  # It will return everything that the user inputs in a list (e.g.: mouse click)
            if event.type == pygame.QUIT or keys[pygame.K_ESCAPE]:  # pygame.quit() checks if we pressed the red X (to leave the app)
                ...
                if 15 <= mouse[0] <= 35 and 5 <= mouse[1] <= 25:
                    elapsed_time = pygame.time.get_ticks() // 1000
                    pause_menu(SCREEN_WIDTH, SCREEN_HEIGHT, elapsed_time, playerCar.lives, coin_counter)
        ...
        for i, line_x in enumerate(dashed_lines_x):
            if i == 0 or i == 2:  # Select the 1st and 3rd lines (assuming 0-based indexing)
                if random.random() < 0.005:  # Fixed probability for coin generation
