...
def gameMP2roads(SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    car2 = IncomingCars("Images/Vehicles/IncomingCars/02C.png", 60, 4,
                        road_x_left + lane_width + (lane_width - 50) // 2)
    car3 = IncomingCars("Images/Vehicles/IncomingCars/05C.png", 60, 3,
                        road_x_left + (2 * lane_width) + (lane_width - 50) // 2)
    car4 = IncomingCars("Images/Vehicles/IncomingCars/07C.png", 60, 1,
                        road_x_left + (3 * lane_width) + (lane_width - 50) // 2)

    car5 = IncomingCars("Images/Vehicles/IncomingCars/03C.png", 60, 2, road_x_right + (lane_width - 50) // 2)
    car6 = IncomingCars("Images/Vehicles/IncomingCars/02C.png", 60, 4,
                        road_x_right + lane_width + (lane_width - 50) // 2)
    car7 = IncomingCars("Images/Vehicles/IncomingCars/05C.png", 60, 3,
                        road_x_right + (2 * lane_width) + (lane_width - 50) // 2)
    car8 = IncomingCars("Images/Vehicles/IncomingCars/07C.png", 60, 1,
                        road_x_right + (3 * lane_width) + (lane_width - 50) // 2)
    ...
    power_up_no_power_up = NoPowerUp("Images/PowerUps/NoPowerUp.png", 50, selected_initial_left_x_pow)
    power_up_key_inversion = KeyInversion("Images/PowerUps/KeyInversion.png", 50, selected_initial_left_x_pow)
    ...
    power_up_no_power_up_right = NoPowerUp("Images/PowerUps/NoPowerUp.png", 50, selected_initial_right_x_pow)
    power_up_key_inversion_right = KeyInversion("Images/PowerUps/KeyInversion.png", 50, selected_initial_right_x_pow)
    ...
    all_sprites_list.add(playerCar1, playerCar2, car1, car2, car3, car4, car5, car6, car7, car8, power_up_invincibility,
                         ...
                         power_up_extra_life, power_up_size_change, power_up_invisible, power_up_no_power_up,
                         power_up_key_inversion, power_up_invincibility_right, power_up_slowing_right,
                         power_up_jet_bomb_right, power_up_extra_life_right, power_up_size_change_right,
                         power_up_invisible_right, power_up_no_power_up_right, power_up_key_inversion_right)
    ...
    incoming_powerups_list_right_road.add(power_up_invincibility_right, power_up_slowing_right, power_up_jet_bomb_right,
                                          ...
                                          power_up_invisible_right,
                                          power_up_no_power_up_right, power_up_key_inversion_right)
    ...
    while carryOn:
        ...
        # Draw player's cars
        if playerCar1.visible:  # boxes só aparecem se houver stored powerup
            player_car_list1.draw(screen)
        if playerCar2.visible:
            player_car_list2.draw(screen)

        # Blit Stored Powerup Box
        if stored_powerup_1 is not None:
        ...
        for powerup in incoming_powerups_list_right_road:
            if powerup.powered_up:
                ...
                if elapsed_time < 100:
                    for car in incoming_cars_list_left:
                        # "car.slowing_mask is not None" is the solution for car.slowing_mask not being
                        # properly initialized or loaded as a pygame.Surface object in the beginning of the game
        ...
        for powerup in incoming_powerups_list_left_road:
            ...
            if powerup.active:
                ...
                if pygame.sprite.collide_mask(playerCar2, powerup) is not None and playerCar2.can_catch_powerup:
                    ...
                    if any(p.powered_up for p in incoming_powerups_list_left_road if
                           p != powerup):  # Checks if any active power ups in list
                        powerup.active = False
                        powerup.powered_up = False
