...
def gameMP(SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    # VEHICLES:
    # Player 1 car
    playerCar1 = PlayerCar("Images/00C.png", 60, 2)
    ...
    # Player 2 car
    playerCar2 = PlayerCar("Images/01C.png", 60, 2)
    ...
    # Opponent cars
    car1 = IncomingCars("Images/03C.png", 60, 2, road_x + (lane_width - 50) // 2)
    car2 = IncomingCars("Images/02C.png", 60, 4, road_x + lane_width + (lane_width - 50) // 2)
    car3 = IncomingCars("Images/05C.png", 60, 3, road_x + (2 * lane_width) + (lane_width - 50) // 2)
    car4 = IncomingCars("Images/07C.png", 60, 1, road_x + (3 * lane_width) + (lane_width - 50) // 2)
    ...
    while carryOn:
        ...
        for event in pygame.event.get():  # It will return everything that the user inputs in a list (e.g.: mouse click)
            if event.type == pygame.QUIT or keys[pygame.K_ESCAPE]:  # pygame.quit() checks if we pressed the red X (to leave the app)
                ...
                if 380 <= mouse[0] <= 400 and 5 <= mouse[1] <= 25:
                    elapsed_time = pygame.time.get_ticks() // 1000
                    pause_menu(SCREEN_WIDTH, SCREEN_HEIGHT, clock, elapsed_time, False, coin_counter=0)
...
def gameMP2roads(SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    # VEHICLES:
    # Player 1 car
    playerCar1 = PlayerCar("Images/00C.png", 60, 3)
    playerCar1.rect.x = right_road_center - (playerCar1.rect.width // 2)  # Center on the left road
    playerCar1.rect.y = SCREEN_HEIGHT - 150

    # Player 2 car
    playerCar2 = PlayerCar("Images/01C.png", 60, 3)
    playerCar2.rect.x = left_road_center - (playerCar2.rect.width // 2)  # Center on the right road
    playerCar2.rect.y = SCREEN_HEIGHT - 150

    # Opponent cars
    car1 = IncomingCars("Images/03C.png", 60, 2, road_x_left + (lane_width - 50) // 2)
    car2 = IncomingCars("Images/02C.png", 60, 4, road_x_left + lane_width + (lane_width - 50) // 2)
    car3 = IncomingCars("Images/05C.png", 60, 3, road_x_left + (2 * lane_width) + (lane_width - 50) // 2)
    car4 = IncomingCars("Images/07C.png", 60, 1, road_x_left + (3 * lane_width) + (lane_width - 50) // 2)
    ...
    all_sprites_list = pygame.sprite.Group()
    incoming_cars_left_list = pygame.sprite.Group()
    incoming_cars_right_list = pygame.sprite.Group()
    ...
    incoming_cars_left_list.add(car1, car2, car3, car4)
    incoming_cars_right_list.add(car5, car6, car7, car8)
    ...
    while carryOn:
        ...
        # Draw the cars
        if spawn_cars_right:
            incoming_cars_right_list.draw(screen)
        if spawn_cars_left:
            incoming_cars_left_list.draw(screen)
        ...
        # Move the opponent cars (automatically):
        for car in incoming_cars_right_list:
            # Velocity
            car.moveDown(playerCar1.speed)
            # Reset cars position when they go off-screen
            if car.rect.y >= SCREEN_HEIGHT and spawn_cars_right:
                car.reshape()

        for car in incoming_cars_left_list:
            # Velocity
            car.moveDown(playerCar2.speed)
            # Reset cars position when they go off-screen
            if car.rect.y >= SCREEN_HEIGHT and spawn_cars_left:
                car.reshape()
        ...
        for car in incoming_cars_right_list:
            ...
            for car_ in incoming_cars_left_list:
